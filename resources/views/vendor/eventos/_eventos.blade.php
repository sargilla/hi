        ...
        <div class="col-md-6 mb-4">
            <div class="card flex-md-row  shadow-sm h-100">
                <div class="card-body d-flex flex-column align-items-start">
                    ...
                    <p class="blog-post-meta">Publicado el {{ $evento->created_at->format('d/m/Y') }} en  <a href="{{ $tipo->path }}">{{ $tipo->nombre }}</a></p>
                    <div class="card-text mb-auto">{!! $evento->contenido_corto !!}</div>
                    <a class="btn btn-primary" href="{{ $evento->path }}">Continuar Leyendo</a>
                </div>
                <img class="card-img-right flex-auto d-none d-lg-block" data-src="/images/thumbnoticia/{{ $evento->imagen_principal }}" alt="{{ $evento->titulo}}" src="/images/thumbnoticia/{{ $evento->imagen_principal }}" data-holder-rendered="true">
            </div>
        </div>
    @empty
