/*
    TEMPLATE
    <div class="form-group" :class="{ 'has-error': form.errors.has('imagenes.archivo') }">
    ...
    data() {
		return {
            urlSubir: route('uploadImagenProducto'),
			urlBorrar: route('borrarImagenProducto'),
        }
    }
    METHODS
    agregarImagen (){
        this.form.imagenes.push({archivo:''})
    },
    borrarImagen(index){
...
<template>
    <div class="d-flex flex-column">
        ...
        <div class="thumbnail" v-else>
            <img src="http://via.placeholder.com/250x250?text=Imagen" width="100%" alt="">
        </div>
        ...
        methods: {
            ...
            borrar(imagenVieja){
                if(this.puedoBorrar || imagenVieja ){
                    axios.post(this.urlBorrar, {nombre: this.imagen})
                    .then(response => {
                        if (imagenVieja){
                            toastr.warning('Se borró la imagen anterior correctamente')
                        } else {
                            toastr.warning('Se borró la imagen correctamente')
                        }
                        this.$emit('update')
                    }).catch(response => {
                        toastr.warning('No se encontró la imagen por lo tanto se borro de la base de datos')
                    })
                    if(!imagenVieja){
                        this.$emit('borrar')
                    }
                } else {
                    toastr.error('Debe quedar al menos una imagen para el producto')
                }
            },
        },
        mounted(){
