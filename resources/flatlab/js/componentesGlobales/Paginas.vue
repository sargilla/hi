	...
	<form class="form-horizontal" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
		<template v-if="evento === 'borrar'">
			<div class="form-group row">
				<label class="control-label col-lg-12" >Seguro que desea borrar esta página?</label>
			</div>
            <div class="form-group row">
				<div class="col-lg-6">
				...
				<div class="col-lg-6">
					<button class="btn btn-primary btn-shadow btn-lg btn-block" :class="{ 'has-spinner': enviando == true }" :disabled="form.errors.any() || enviando == true" >Borrar Página <i class="fas fa-circle-o-notch fa-spin" v-show="enviando == true"></i> </button>
				</div>
			</div>
		</template>
		<template v-else>
            <div class="overlay-buttons">
                ...
                <button data-toggle="collapse" data-target=".modulos-body" class="tooltips btn btn-warning btn-shadow" type="button" title="Abrir/Cerrar Paneles">
            ...
            <div class="sb-slidebar sb-right sb-style-overlay">
                ...
                <div class="container checkboxes" v-if="user.can['paginas-crearmodulos']">
                    <label class="label_check" :class="isChecked(posicion.value) ? 'c_on' : 'c_off'" v-for="(posicion,index) in posiciones" :key="index">
                        <input v-model="form.posiciones" :value="posicion.value" type="checkbox" >{{ posicion.nombre }}
                    </label>
                </div>
                ...
                <div class="container" v-if="user.can['paginas-crearmodulos']">
                    ...
                    <div class="form-group">
                        <select v-model="posicion_elegida" class="custom-select">
                            <option disabled value="">Elija una posicion</option>
                            <option value="contenido">Contenido</option>
                            <option v-for="(posicion,index) in posiciones" :key="index" :value="posicion.value" v-text="posicion.nombre"></option>
                        </select>
                    </div>
                    <div class="form-group">
                        <span class="input-group-btn">
                            <button class="btn btn-primary btn-block" @click.prevent="agregarModulo">Agregar Modulo</button>
                        </span>
                    </div>
                </div>
			...
			<div class="form-group row"
				...
				<label for="titulo" class="col-md-2 col-form-label" >Titulo de la Página</label>
				<div class="col-md-10">
					<input class="form-control" v-model="form.titulo" required>
			...
			<div class="" v-if="isChecked('encabezado')">
				...
				<draggable v-model="form.contenido" @start="drag=true" @end="drag=false" handle=".my-handle">
					<component :is="modulo.tipo" v-for="(modulo,index) in form.encabezado" :id="`m-${index}`" :key="index" :campos.sync="modulo.campos" :nombre.sync="modulo.nombre" :estilo.sync="modulo.estilo"  @borrar="form.encabezado.splice(index, 1)"></component>
				</draggable>
			</div>
			<div class="">
				...
				<draggable v-model="form.contenido" @start="drag=true" @end="drag=false" handle=".my-handle">
					<component :is="modulo.tipo" v-for="(modulo,index) in form.contenido" :id="`m-${index}`" :key="index" :campos.sync="modulo.campos" :nombre.sync="modulo.nombre" :estilo.sync="modulo.estilo"  @borrar="form.contenido.splice(index, 1)"></component>
				</draggable>
                </div>
			</div>
            <div class="" v-if="isChecked('derecha')">
				...
				<draggable v-model="form.contenido" @start="drag=true" @end="drag=false" handle=".my-handle">
					<component :is="modulo.tipo" v-for="(modulo,index) in form.derecha" :id="`m-${index}`" :key="index" :campos.sync="modulo.campos" :nombre.sync="modulo.nombre" :estilo.sync="modulo.estilo"  @borrar="form.derecha.splice(index, 1)"></component>
				</draggable>
			</div>
             <div class="" v-if="isChecked('izquierda')">
				<h4 class="text-center p-2 border-bottom">Izquierda</h4>
				<draggable v-model="form.contenido" @start="drag=true" @end="drag=false" handle=".my-handle">
					<component :is="modulo.tipo" v-for="(modulo,index) in form.iquierda" :id="`m-${index}`" :key="index" :campos.sync="modulo.campos" :nombre.sync="modulo.nombre" :estilo.sync="modulo.estilo"  @borrar="form.iquierda.splice(index, 1)"></component>
				</draggable>
			</div>
             <div class="" v-if="isChecked('pie')">
				<h4 class="text-center p-2 border-bottom">Pie</h4>
				<draggable v-model="form.contenido" @start="drag=true" @end="drag=false" handle=".my-handle">
					<component :is="modulo.tipo" v-for="(modulo,index) in form.pie" :id="`m-${index}`" :key="index" :campos.sync="modulo.campos" :nombre.sync="modulo.nombre" :estilo.sync="modulo.estilo"  @borrar="form.pie.splice(index, 1)"></component>
				</draggable>
			</div>
		</template>
	</form>
	...
	export default {
		...
		data() {
	        return {
	        	...
	        	modulos: [
                    ...
                    {nombre:'Titulo con Texto e Imágen', value:'titulo_texto_imagen'},
		...
		methods: {
            ...
            agregarModulo() {
            	...
            	this.form[this.posicion_elegida].push({nombre:'',tipo: this.modulo_elegido, campos:{}})
            },
            onClick(value) {
                this.quedarse = value
            },
			onSubmit() {
				...
				if(this.evento == 'borrar'){
					axios.delete(route('paginas.destroy',this.form.id))
		           	.then( response => this.resetModal(response.data.mensaje))
				}
			},
			resetModal(mensaje){
				$('#paginas').DataTable().draw(false)
		...
		mounted() {
			...
			if(this.evento == 'editar'){
				...
				this.form.encabezado = (this.pagina.encabezado) ? this.pagina.encabezado : []
				this.form.derecha = (this.pagina.derecha) ? this.pagina.derecha : []
				this.form.posiciones = (this.pagina.posiciones) ? this.pagina.posiciones : []
            }
		},
		computed: {
